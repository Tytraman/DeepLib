...
namespace deep
{
    ...
    template struct DEEP_LIB_API vec3<uint8>;
    template struct DEEP_LIB_API vec3<float>;

#define DEEP_DECLARE_COLOR(__name, __r, __g, __b) static constexpr color_rgb __name = color_rgb(DEEP_U8(__r), DEEP_U8(__g), DEEP_U8(__b))

    class DEEP_LIB_API color
    {
      public:
        static color_rgb find_closest_color(const color_rgb &color, const color_rgb *colors, usize number_of_colors);
        static color_linearized_rgb linearise(const color_rgb &color);
        static color_rgb delinearise(const color_linearized_rgb &color);
        static float calc_luminance(const color_linearized_rgb &color);
